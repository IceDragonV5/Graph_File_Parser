...
class WeightedGraph(Graph) :
    ...
    def bellmanFord(self,s) :
        ...
        for u in range(self.numVertices()): #init-single-source
            parent.append(None)
            weight.append(float('inf'))
        weight[s] = 0
        
        #listEdges = self.getEdgeList(True)

        for i in range(self.numVertices()-1):
            for u in range(self.numVertices()):
                for v, w in self._adj[u].__iter__(True): #relax
                    if weight[u] != float('inf') and (weight[v] > weight[u] + w):
                        weight[v] = weight[u] + w
                        parent[v] = u
        for u in range(self.numVertices()):
            for v, w in self._adj[u].__iter__(True): #check for negative weight cycles
                if weight[u] != float('inf') and (weight[v] > weight[u] + w):
                    return []
        
        for u in range(self.numVertices()):
            retList.append( (u, weight[u], parent[u]) )
        return retList
    ...
    def dijkstra(self,s) :
        ...
        queue = PQInts(self.numVertices())
        #for u in range(self.numVertices()):
        #    if u != s:
        #        queue.insert(u, weight[u])
        for u in range(self.numVertices()):
            queue.insert(u, weight[u])
        while not queue.isEmpty():
            ...
            for v, w in self._adj[u].__iter__(True): #relax
                if weight[v] > weight[u] + w:
                    weight[v] = weight[u] + w
                    parent[v] = u
                    queue.changePriority(v, weight[v])
