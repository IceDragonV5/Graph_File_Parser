...
#    or as a separate text file: see answers.txt
...
def timeShortestPathAlgs() :
    ...
    dj = []
    out = []
    verts = [16, 64, 256, 16, 64, 256]
    edges = [120, 2016, 32640, 32, 128, 512]
    for i in range(6):
        bf.append( "{0:.5}".format(timeit(lambda: randomWeightedGraph(verts[i], edges[i], 1, 10).bellmanFord(0), number = num)) )
        dj.append( "{0:.5}".format(timeit(lambda: randomWeightedGraph(verts[i], edges[i], 1, 10).dijkstra(0), number = num)) )

    for i in range(6):
        out.append(f"{verts[i]:<10}{edges[i]:<10}{bf[i]:<15}{dj[i]:>15}")
...
if __name__ == "__main__" :
    ...
    g = WeightedGraph(5, [ (0,1), (0,3), (1,2), (1,3), (2,4), (3,1), (3,2), (3,4), (4,0), (4,2) ], [ 10, 5, 1, 2, 4, 3, 9, 2, 7, 6 ])
    print(g.bellmanFord(0))
    print(g.dijkstra(0))

    gNegative = WeightedGraph(3, [(0,1), (1,2), (2,0)], [1, -1, -1]) #testing a negative weight cycle in bellman ford
    print(gNegative.bellmanFord(0))

    print(f"{'Vertices':<10}{'Edges':<10}{'Bellman Ford':<15}{'Dijkstra':>15}")
    outList = timeShortestPathAlgs()
